...
select="$timeSeriesRecordPath[1]/gml:valueComponents/ioos:Array/gml:valueComponents/ioos:Composite[1]//ioos:CompositeValue[1]"/>
  ...
  <xsl:template name="reformatZuluTime">
    <xsl:param name="value"/>
    <xsl:value-of select="translate(translate($value,'Z',''),'T',' ')" />
	</xsl:template>
	...
	<xsl:template name="binObservation">
      ...
      <xsl:variable name="fixedtime">
        <xsl:call-template name="reformatZuluTime">
          <xsl:with-param name="value" select="$timePosition"/>
        </xsl:call-template>
      </xsl:variable>
			<xsl:value-of select="$fixedtime"/>
