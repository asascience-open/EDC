...
class Bean {
  ...
  private BeanParser p = null; // the bean parser (shared for all beans of
  // same class)
  ...
  public Bean(org.xml.sax.Attributes atts) throws ClassNotFoundException, InstantiationException,
  ...
  static class Collection {
    ...
    private java.lang.Class beanClass; // the class of the beans in the
    // collection
    private BeanParser p = null; // the bean parser (shared for all beans of
    // same class)
    ...
    Collection(org.xml.sax.Attributes atts) throws ClassNotFoundException { // ,
  ...
  private static class BeanParser {
    ...
    BeanParser(Class beanClass) {
      ...
      if (debugBean) {
        System.err.println("Bean " + beanClass.getName());
      }
      ...
      for (int i = 0; i < pds.length; i++) {
        if ((pds[i].getReadMethod() != null) && (pds[i].getWriteMethod() != null)) {
          ...
          if (debugBean) {
            System.err.println(" property " + pds[i].getName());
          }
        }
      }
    }
      ...
      while (iter.hasNext()) {
        ...
        try {
          ...
          if (value instanceof String) {
            value = XMLStorage.quote((String) value);
          } else if (value instanceof Date) {
            value = new Long(((Date) value).getTime());
          }
          out.print(pds.getName() + "='" + value + "' ");
          if (debugBean) {
            System.err.println(" property get " + pds.getName() + "='" + value + "' ");
          }
    ...
    void readProperties(Object bean, org.xml.sax.Attributes atts) {
      ...
      while (iter.hasNext()) {
        ...
        try {
          ...
          if (debugBean) {
            System.err.println(" property set " + pds.getName() + "=" + sArg + " == " + arg);
          }
          if (arg == null) {
            return;
          }
