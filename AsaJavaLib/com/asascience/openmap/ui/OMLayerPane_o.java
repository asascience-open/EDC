 ...
 */
public class OMLayerPane_o extends JPanel implements ActionListener {
   ...
   * @param tlh
   */
  public OMLayerPane_o(Layer inLayer, LayerHandler lh, OMTimeSlider tlh) {
    ...
    String name = (layer.getName().lastIndexOf(File.separator) == -1) ? layer.getName() : layer.getName().substring(layer.getName().lastIndexOf(File.separator) + 1);
    ...
    if (cmd.equals("visible")) {
      ...
      if (mb != null) {
        ...
        if (rect != null) {
          ...
          float scale = com.bbn.openmap.proj.ProjMath.getScale(MapUtils.ucarPointToBbnPoint(rect.getLowerLeftPoint()), MapUtils.ucarPointToBbnPoint(rect.getUpperRightPoint()), proj);
      ...
      if (layer instanceof ShapeLayer) {
        ...
        String outS = (String) JOptionPane.showInputDialog(this.getTopLevelAncestor(),
                "Choose a file for the Basemap layer:", "Select Basemap", JOptionPane.PLAIN_MESSAGE, null, names.toArray(), (names.contains(nowFile) ? nowFile : names.get(0)));
