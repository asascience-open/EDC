/*
 * @(#)DeterminateProgressMonitor.java	1.30 04/04/15
 *
 ...
 * DeterminateProgressMonitor is created it is given a numeric range and a
 * descriptive string. As the operation progresses, call the setProgress method
 * to indicate how far along the [min,max] range the operation is. Initially,
 * there is no ProgressDialog. After the first millisToDecideToPopup
 * milliseconds (default 500) the progress monitor will predict how long the
 * operation will take. If it is longer than millisToPopup (default 2000, 2
 * seconds) a ProgressDialog will be popped up.
 * <p>
 ...
 */
public class DeterminateProgressMonitor extends Object implements Accessible {

  private DeterminateProgressMonitor root;
   ...
   * @see JOptionPane
   */
  public DeterminateProgressMonitor(Component parentComponent, Object message, String note, int min, int max,
          boolean modal, boolean resizable, ImageIcon monitorIcon) {
    this(parentComponent, message, note, min, max, null, modal, resizable, monitorIcon);
  }

  private DeterminateProgressMonitor(Component parentComponent, Object message, String note, int min, int max,
          DeterminateProgressMonitor group, boolean modal, boolean resizable, ImageIcon monitorIcon) {
    this.min = min;
    this.max = max;
    this.modal = modal;
  ...
  private class ProgressOptionPane extends JOptionPane {
    ...
    ProgressOptionPane(Object messageList) {
      super(messageList, JOptionPane.INFORMATION_MESSAGE, JOptionPane.DEFAULT_OPTION, monitorIcon,
              DeterminateProgressMonitor.this.cancelOption, null);
    }
    ...
    public JDialog createDialog(Component parentComponent, String title) {
      ...
      if (parentComponent instanceof JFrame) {
        dialog = new JDialog((JFrame) parentComponent, title, modal);
      } else if (parentComponent instanceof JDialog) {
        dialog = new JDialog((JDialog) parentComponent, title, modal);
      } else {
      ...
      /** Removed from original due to accessibility issues **/
      // Window window =
      // JOptionPane.getWindowForComponent(parentComponent);
    ...
    public AccessibleContext getAccessibleContext() {
      return DeterminateProgressMonitor.this.getAccessibleContext();
    }
  ...
  private String calcString(int val) {
    double per = 0;
    if (val > 0) {
      per = ((double) val / (double) max) * 100d;
    }
  ...
  public void setProgress(int nv) {
    // System.out.println("setProgress=" + nv);
    if (myBar != null) {
      myBar.setValue(nv);
      myBar.setString(calcString(nv));
    } else {
   ...
   * <code>DeterminateProgressMonitor</code>
   *
   * @since 1.5
   */
  ...
  public AccessibleContext getAccessibleContext() {
    ...
    if (pane != null && accessibleJOptionPane == null) {
      ...
      // the DeterminateProgressMonitor does not have a Component
   ...
   * for the <code>DeterminateProgressMonitor</code> class.
   *
   * @since 1.5
   */
  protected class AccessibleProgressMonitor extends AccessibleContext implements AccessibleText, ChangeListener,
    ...
    /*
     * The accessibility hierarchy for DeterminateProgressMonitor is a
     * flattened version of the ProgressOptionPane component hierarchy.
     *
     ...
     * been created. Because the DeterminateProgressMonitor is not a
     * Component implementing the Accessible interface, an AccessibleContext
